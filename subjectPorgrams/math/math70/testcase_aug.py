...
class RuntimeContext(object):
    ...
    def __init__(self):
        ...
        self.covMatrix_global = config_parser.get(coverage_information_matrix_section, "covMatrix_global")
...
def main():
    ...
    f4 = open(runtime_context.covMatrix_global, 'r')
    """ 
    set parameters
    """
    ...
    first_ele = True
    for data in f4.readlines():
        data = data.strip('\n')
        nums = data.split()
        if first_ele:
            nums = [float(x) for x in nums]
            matrix_x_global = np.array(nums)
            first_ele = False
        else:
            nums = [float(x) for x in nums]
            matrix_x_global = np.c_[matrix_x_global,nums]
    f4.close()
    
    matrix_x = matrix_x.transpose()
    matrix_y = matrix_y.transpose()
    matrix_x_global = matrix_x_global.transpose()
    ...
    for testcase_num in range(len(matrix_y)):
        if matrix_y[testcase_num][0] == 1:
            inputs_global_pre.append(matrix_x_global[testcase_num])
            testcase_fail_num = testcase_fail_num + 1
    TESTNUM = testcase_fail_num
    inputs_global = torch.FloatTensor(inputs_global_pre)
    ...
    for item in fake_testcase_numpy:       
        for element_num in range(len(item)):
            if item[element_num] <= 0.5 and minimum_suspicious_set[element_num] == 0:
                f3.write('0.0')
            if item[element_num] <= 0.5 and minimum_suspicious_set[element_num] == 1:
                f3.write('1.0')
            if item[element_num] > 0.5 and minimum_suspicious_set[element_num] == 1:
                f3.write(str(round(item[element_num],2)*inputs_global[inputs_global_num%len(inputs_global)][element_num].numpy()))
            if item[element_num] > 0.5 and minimum_suspicious_set[element_num] == 0:
                f3.write(str(round(item[element_num],2)*inputs_global[inputs_global_num%len(inputs_global)][element_num].numpy()))            
            f3.write(' ')
        f3.write('\n')
        inputs_global_num = inputs_global_num + 1
    logger.info(fake_testcase_numpy)
    logger.info(minimum_suspicious_set)
#    print(inputs_global)
    f3.close()
    """
    save model
